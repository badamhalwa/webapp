...
const PATIENTS_NAV = [
  { label: 'Book Appointment',   path: '/patient/book',        Icon: FiCalendar },
  { label: 'Track Appointment',  path: '/patient/track',       Icon: FiSearch },
  { label: 'Live Queue',         path: '/patient/queue',       Icon: FiClock },
  { label: 'Patient Follow-Up',  path: '/patient/followup',    Icon: FiClipboard },
  { label: 'Hospital Directions',path: '/patient/directions',  Icon: FiMapPin },
  { label: 'Feedback',           path: '/patient/feedback',    Icon: FiMessageSquare },
];

const ACADEMICS_NAV = [
  { label: 'Student Dashboard', path: '/student/dashboard', Icon: FiUser },
  { label: 'Syllabus',          path: '/student/syllabus',  Icon: FiBook },
  { label: 'Schedule',          path: '/student/schedule',  Icon: FiCalendar },
  { label: 'Hostel Complaints', path: '/student/hostel',    Icon: FiHome },
  { label: 'Admissions',        path: '/admissions',        Icon: FiBookOpen },
];

const MORE_NAV = [
  { label: 'Achievements',     path: '/achievements',  Icon: FiAward },
  { label: 'Virtual Tour',     path: '/tour',          Icon: FiMap },
  { label: 'Events',           path: '/events',        Icon: FiCalendar },
  { label: 'PG Doctor Portal', path: '/faculty/pg',    Icon: FiActivity },
  { label: 'Admin Dashboard',  path: '/faculty/admin', Icon: FiShield },
];
    ...
    <>
      ...
      <div className="topbar hidden md:flex">
        <div className="topbar-links">
          ...
          <span className="topbar-link flex items-center gap-1">
            <FiMapPin size={10} /> Mysuru Road, Kambipura, Bangalore
          </span>
        </div>
        <div className="flex items-center gap-3">
              ...
              {notifOpen && (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid #eaecf0', background: '#003580', borderRadius: '10px 10px 0 0' }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#fff', fontFamily: 'Manrope, sans-serif', display: 'flex', alignItems: 'center', gap: 6 }}><FiBell size={12} /> Live Updates</span>
                  ...
                  {notifications.length === 0
                    ? <p style={{ padding: '16px 14px', fontSize: 12, color: '#777', textAlign: 'center', fontFamily: 'Inter, sans-serif' }}>No updates yet.</p>
                    : notifications.map(n => (
          ...
          <Link to="/patient/book">
            <button className="topbar-btn">Book Appointment</button>
          </Link>
        </div>
      </div>
      ...
      <nav className="navbar" role="navigation" aria-label="Main navigation">
        ...
        <div className="hidden md:flex items-center" style={{ height: '64px' }}>
          ...
          <NavDropdown label="Patients"  links={PATIENTS_NAV}  active={isPatientsActive} />
          <NavDropdown label="Academics" links={ACADEMICS_NAV} active={isAcademicsActive} />
          <NavDropdown label="More"      links={MORE_NAV}      active={false} />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/patient/book" className="md:hidden">
            <button className="topbar-btn" style={{ fontSize: 11, padding: '5px 10px' }}>Book</button>
          </Link>
          ...
          >
            <form onSubmit={handleSearch} style={{ display: 'flex', gap: 8, padding: '12px 2rem', maxWidth: '700px', margin: '0 auto' }}>
              <input
                ...
                aria-label="Search"
              />
              <button type="submit" className="vs-btn vs-btn-primary" style={{ borderRadius: 5, padding: '9px 18px' }}>Search</button>
